...
def load_nifty_data(file_path, batch_size=6):
    ...
    mean_train, std_train = data_weather.compute_nifty_returns_statistic(data_file, start_time='2013-3-1 0:0',
                                                                    end_time='2016-10-30 23:0')
    train_loader = data_weather.get_nifty_data(data_file, start_time='2013-3-6 0:0',
                                                 end_time='2015-5-31 23:0', batch_size=batch_size, mean=mean_train, std=std_train)
    valid_train_loader = data_weather.get_nifty_data(data_file, start_time='2015-6-2 0:0',
                                                       end_time='2016-6-30 23:0', batch_size=batch_size, mean=mean_train, std=std_train)
    valid_vld_loader = data_weather.get_nifty_data(data_file, start_time='2016-7-2 0:0',
                                                     end_time='2016-10-30 23:0', batch_size=batch_size, mean=mean_train, std=std_train)
    test_loader = data_weather.get_nifty_data(data_file, start_time='2016-11-2 0:0',
        ...
        for i in range(1, len(selected)):
            if i == 1:
                sel_start_time = start_time + datetime.timedelta(
                    days=int(num_day / split_N * selected[i - 1]), hours=0)
            else:
                sel_start_time = start_time + datetime.timedelta(
                    days=int(num_day / split_N * selected[i - 1]) + 1, hours=0)
...
def load_nifty_data_multi_domain(file_path, batch_size=6, number_domain=2, mode='pre_process', dis_type='coral'):
    ...
    for i in range(len(split_time_list)):
        ...
        train_loader = data_weather.get_nifty_data(data_file, start_time=time_temp[0],
